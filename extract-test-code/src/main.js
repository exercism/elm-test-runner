...
var $author$project$ExtractTestCode$extractFromTestFunction = function (expressions) {
	if ((((expressions.b && (expressions.a.$ === 'Literal')) && expressions.b.b) && (expressions.b.a.$ === 'LambdaExpression')) && (!expressions.b.b.b)) {
		...
		var test = _v1.a.a;
		var _v2 = A2($elm$core$Debug$log, 'test', 'test');
		return _List_fromArray(
			[
				_Utils_Tuple2(
				name,
				$stil4m$elm_syntax$Elm$Syntax$Node$value(test.expression))
			]);
	} else {
		return _List_Nil;
	}
...
var $author$project$ExtractTestCode$extractFromExpression = function (expression) {
	...
	while (true) {
		switch (expression.$) {
			case 'Application':
				var nodeExpressions = expression.a;
				var expressions = A2($elm$core$List$map, $stil4m$elm_syntax$Elm$Syntax$Node$value, nodeExpressions);
				if (expressions.b && (expressions.a.$ === 'FunctionOrValue')) {
					var _v2 = expressions.a;
					var functionName = _v2.b;
			...
			case 'OperatorApplication':
				...
				var direction = expression.b;
				var left = expression.c;
				var right = expression.d;
				var _v5 = $stil4m$elm_syntax$Elm$Syntax$Node$value(left);
				if (_v5.$ === 'Application') {
					var nodeExpressions2 = _v5.a;
					var $temp$expression = $stil4m$elm_syntax$Elm$Syntax$Expression$Application(
						_Utils_ap(
							nodeExpressions2,
							_List_fromArray(
								[right])));
...
var $author$project$Main$main = $elm$core$Platform$worker(
	{init: $author$project$Main$init2, subscriptions: $author$project$Main$subscriptions, update: $author$project$Main$update});
